...
/**
 * Selenium тест для страницы списка пользователей.
 */
@SpringBootTest(
...
public class UserListTest {
    ...
    /**
     * Создание пользователя с правами ADMIN перед каждым тестом.
     */
    @BeforeEach
    public void ensureAdminUser() {
        ...
        try {
            u.getClass()
                    .getMethod("setCredentialsNonExpired", boolean.class)
                    .invoke(u, true);
        } catch (Exception ignored) {
        }
    ...
    /**
     * Тест доступа к странице списка пользователей.
     */
    @Test
    public void userListPage() {
        String loginUrl = "http://localhost:" + port + "/login";
        String userListUrl = "http://localhost:" + port + "/userlist";
        driver.get(loginUrl);
        ...
        wait.until(ExpectedConditions.presenceOfElementLocated(
                By.tagName("h1")));
        assertTrue(Objects.requireNonNull(driver.getPageSource())
                .contains("Список пользователей"));
